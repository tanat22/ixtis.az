...
import { X, PlusCircle, ArrowUp, ArrowDown } from 'lucide-react';
...
export default function CodingPage() {
  ...
  React.useEffect(() => {
    const applyFilters = () => {
      let tempSpecialties = specialties.filter(
        (s) => s.level === educationLevel
      );
  ...
  const getUniversityName = (id: number) =>
    universities.find((u) => u.id === id)?.name || 'Naməlum';
  ...
  return (
    <main className="container mx-auto grid max-w-7xl grid-cols-1 items-start gap-8 px-4 py-8 sm:px-8 lg:grid-cols-3">
      <div className="space-y-8 lg:col-span-2">
        <Card>
          <CardHeader>
            <CardTitle>Filtrlər</CardTitle>
            <CardDescription>
              Axtarışınızı dəqiqləşdirin.
            </CardDescription>
          </CardHeader>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={() => handleTypeChange('Dövlət sifarişli')}
                  />
                  ...
                  filteredSpecialties.map((spec) => (
                    <TableRow key={spec.id}>
                      ...
                      <TableCell>
                        <Button
                          size="sm"
                          onClick={() => handleAddToSelection(spec)}
                          disabled={
                            !!selectedList.find((s) => s.id === spec.id) ||
      ...
      <div className="sticky top-24 lg:col-span-1">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Maksimum 15 ixtisas seçə bilərsiniz. ({selectedList.length}/15)
            </CardDescription>
          </CardHeader>
                ...
                {selectedList.map((spec, index) => (
                  ...
                  >
                    <div className="flex-1">
                       <Link href={`/faculty/${spec.slug}`} className="hover:underline text-sm font-semibold">
                         {index + 1}. {spec.name}
                      </Link>
                    ...
                    <div className="flex items-center gap-1">
                      <Button
                        ...
                        size="icon"
                        className="h-7 w-7"
                        onClick={() => moveSelection(index, 'up')}
                        disabled={index === 0}
                      >
                      ...
                      <Button
                        ...
                        size="icon"
                        className="h-7 w-7"
                        onClick={() => moveSelection(index, 'down')}
                      ...
                      <Button
                        ...
                        onClick={() => handleRemoveFromSelection(spec.id)}
                      >
                        <X className="h-4 w-4" />
                      </Button>
                    </div>
          ...
          {selectedList.length > 0 && (
            <CardFooter>
              <Button className="w-full">Seçimləri Təsdiqlə</Button>
            </CardFooter>
          )}
        </Card>
      </div>
